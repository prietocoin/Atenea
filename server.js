  ...
  <script>
    tailwind.config = {
      ...
      theme: {
        extend: {
          colors: {
            brand: { dark: '#0d0f1d', card: '#16192b', border: '#262a42', accent: '#8b5cf6' }
          }
        }
      }
    }
              ...
              <table class="w-full text-xs text-left border-collapse">
                <thead>
                  <tr class="text-slate-400 border-b border-slate-800 bg-[#171a2e] uppercase font-bold text-[10px] tracking-wider">
                    ...
                    <th class="px-3 py-2.5">SOCIO 1 & TIPO</th>
                ...
                <tbody class="divide-y divide-slate-800/60 font-medium text-[11px]">
                  <template x-for="item in comprobantes" :key="item.hash_largo">
                    <tr class="hover:bg-slate-800/30 transition-colors">
                      ...
                      <!-- HASH + LÁPIZ TÁCTIL (SIN COLUMNA EXTRA) -->
                      <td class="px-3 py-2 align-middle whitespace-nowrap">
      ...
      <div x-show="modalAbierto" class="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/80 backdrop-blur-sm" x-cloak>
        <div class="bg-[#131525] border border-slate-700/80 rounded-2xl w-full max-w-md p-4 space-y-3 shadow-2xl">
          ...
          <div class="flex items-center justify-between border-b border-slate-800 pb-2">
            <div class="flex items-center gap-2">
              <h3 class="font-bold text-white text-sm">Auditar Comprobante</h3>
              <span class="text-xs text-purple-400 font-mono font-bold" x-text="itemEdicion ? itemEdicion.hash_corto : ''"></span>
              
              <!-- INSIGNIA D/P/A/C DENTRO DEL MODAL -->
              <template x-if="itemEdicion && getCodigoOperativo(itemEdicion.nombre_socio_1, itemEdicion.moneda)">
                <span 
                  :class="claseInsignia(getCodigoOperativo(itemEdicion.nombre_socio_1, itemEdicion.moneda))"
                  class="px-2 py-0.5 rounded text-[10px] font-black border uppercase tracking-wider"
                  x-text="'TIPO ' + getCodigoOperativo(itemEdicion.nombre_socio_1, itemEdicion.moneda)">
                </span>
              </template>
            </div>
            ...
            <div class="space-y-2.5">
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <!-- SELECT DE MONEDAS COMPLETO -->
                  <select x-model="itemEdicion.moneda" class="w-full bg-[#1a1d33] border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-white uppercase outline-none focus:border-purple-500">
                    <option value="">Seleccionar</option>
                    <template x-for="m in listaMonedas" :key="m">
                      <option :value="m" x-text="m" :selected="m === itemEdicion.moneda"></option>
                    </template>
                  </select>
                </div>
              </div>
  ...
  <script>
    function app() {
      return {
        ...
        // TODAS LAS MONEDAS DE AMÉRICA + EUR + USDT
        listaMonedas: [
          'USD', 'EUR', 'USDT', 'PEN', 'COP', 'CLP', 'ARS', 'MXN', 'BRL', 'VES',
          'PYG', 'DOP', 'CRC', 'CAD', 'ECU', 'PAN', 'BOB', 'UYU', 'GTQ', 'HNL',
          'NIO', 'PAB'
        ],
        ...
        // CRUCE SOCIO 1 VS MONEDA COMPROBANTE (BUSCA EN CUALQUIER COLUMNA DE MONEDA)
