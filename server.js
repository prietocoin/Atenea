...
// CONFIGURACIÓN ROBUSTA DEL POOL (Previene crasheos en Coolify)
const pool = new Pool({
...
// 🛡️ ESCUDO ANTI-CRASHEO: Evita que Node.js se apague si Postgres corta una conexión inactiva
pool.on('error', (err, client) => {
  console.error('⚠️ Error inesperado en el cliente de PostgreSQL (inactivo):', err.message);
});
...
// -----------------------------------------------------------------------------
// HEALTH CHECK (Requerido por Coolify para saber si el contenedor está vivo)
// -----------------------------------------------------------------------------
app.get('/health', (req, res) => {
  res.status(200).send('Servidor Atenea Activo y Saludable');
});
